...
output "java_vm_ip" {
  value = (
    var.deploy_mode == "linux" ? vsphere_virtual_machine.vm["java-vm"].default_ip_address :
    var.deploy_mode == "windows" ? vsphere_virtual_machine.win_vm["win-java-vm"].default_ip_address :
    ""
  )
}
...
output "dotnet_vm_ip" {
  value = (
    var.deploy_mode == "linux" ? vsphere_virtual_machine.vm["dotnet-vm"].default_ip_address :
    var.deploy_mode == "windows" ? vsphere_virtual_machine.win_vm["win-dotnet-vm"].default_ip_address :
    ""
  )
}
...
output "php_vm_ip" {
  value = (
    var.deploy_mode == "linux" ? vsphere_virtual_machine.vm["php-vm"].default_ip_address :
    var.deploy_mode == "windows" ? vsphere_virtual_machine.win_vm["win-php-vm"].default_ip_address :
    ""
  )
}
...
output "java_3tier_ips" {
  ...
  value = var.deploy_mode == "linux-3tier" ? {
