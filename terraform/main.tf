...
locals {
  ...
  # --- 3-Tier Linux VMs (9 VMs) ---
  vms_3tier = var.deploy_mode == "linux-3tier" ? {
    java-fe = {
      name   = "3t-java-fe"
      cpus   = 1
      memory = 2048
  ...
  # --- 3-Tier Windows VMs (9 VMs) ---
  win_vms_3tier = var.deploy_mode == "windows-3tier" ? {
    win-java-fe = {
      name          = "3t-win-java-fe"
...
resource "local_file" "ansible_inventory" {
  content = templatefile("${path.module}/templates/hosts.ini.tftpl", {
    ...
    # 3-tier Linux IPs
    java_fe_ip    = var.deploy_mode == "linux-3tier" ? vsphere_virtual_machine.vm_3tier["java-fe"].default_ip_address : ""
    java_app_ip   = var.deploy_mode == "linux-3tier" ? vsphere_virtual_machine.vm_3tier["java-app"].default_ip_address : ""
    java_db_ip    = var.deploy_mode == "linux-3tier" ? vsphere_virtual_machine.vm_3tier["java-db"].default_ip_address : ""
    dotnet_fe_ip  = var.deploy_mode == "linux-3tier" ? vsphere_virtual_machine.vm_3tier["dotnet-fe"].default_ip_address : ""
    dotnet_app_ip = var.deploy_mode == "linux-3tier" ? vsphere_virtual_machine.vm_3tier["dotnet-app"].default_ip_address : ""
    dotnet_db_ip  = var.deploy_mode == "linux-3tier" ? vsphere_virtual_machine.vm_3tier["dotnet-db"].default_ip_address : ""
    php_fe_ip     = var.deploy_mode == "linux-3tier" ? vsphere_virtual_machine.vm_3tier["php-fe"].default_ip_address : ""
    php_app_ip    = var.deploy_mode == "linux-3tier" ? vsphere_virtual_machine.vm_3tier["php-app"].default_ip_address : ""
    php_db_ip     = var.deploy_mode == "linux-3tier" ? vsphere_virtual_machine.vm_3tier["php-db"].default_ip_address : ""
    # 3-tier Windows IPs
    win_java_fe_ip    = var.deploy_mode == "windows-3tier" ? vsphere_virtual_machine.win_vm_3tier["win-java-fe"].default_ip_address : ""
    win_java_app_ip   = var.deploy_mode == "windows-3tier" ? vsphere_virtual_machine.win_vm_3tier["win-java-app"].default_ip_address : ""
    win_java_db_ip    = var.deploy_mode == "windows-3tier" ? vsphere_virtual_machine.win_vm_3tier["win-java-db"].default_ip_address : ""
    win_dotnet_fe_ip  = var.deploy_mode == "windows-3tier" ? vsphere_virtual_machine.win_vm_3tier["win-dotnet-fe"].default_ip_address : ""
    win_dotnet_app_ip = var.deploy_mode == "windows-3tier" ? vsphere_virtual_machine.win_vm_3tier["win-dotnet-app"].default_ip_address : ""
    win_dotnet_db_ip  = var.deploy_mode == "windows-3tier" ? vsphere_virtual_machine.win_vm_3tier["win-dotnet-db"].default_ip_address : ""
    win_php_fe_ip     = var.deploy_mode == "windows-3tier" ? vsphere_virtual_machine.win_vm_3tier["win-php-fe"].default_ip_address : ""
    win_php_app_ip    = var.deploy_mode == "windows-3tier" ? vsphere_virtual_machine.win_vm_3tier["win-php-app"].default_ip_address : ""
    win_php_db_ip     = var.deploy_mode == "windows-3tier" ? vsphere_virtual_machine.win_vm_3tier["win-php-db"].default_ip_address : ""
  })
