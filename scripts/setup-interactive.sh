...
load_previous() {
    ...
    if [[ -f "$TFVARS_FILE" ]]; then
        ...
        PREV_WIN_CLN_1TIER_IP="$(tfval win_cln_1tier_ip "")"
        PREV_WIN_CLN_1TIER_HOSTNAME="$(tfval win_cln_1tier_hostname "win-dotnet-cln")"
    ...
    if [[ "$DEPLOY_DOTNET" == "true" ]]; then
        ...
        if [[ "$DEPLOY_LINUX_3TIER" == "true" || "$DEPLOY_WINDOWS_3TIER" == "true" ]]; then
            ...
            if [[ "$REPLY" == "2" ]]; then
                ...
                echo ""
                echo -e "  ${Y}Note:${NC} CleanArchitecture deploys onto its OWN separate .NET 3-tier"
                echo -e "        VM set (frontend / app / database) that coexists with any existing"
                echo -e "        eShop .NET VMs — it does not replace or destroy them. The .NET"
                echo -e "        3-tier hostnames/IPs you provide become the CleanArchitecture VMs;"
                echo -e "        existing eShop VMs in Terraform state are preserved as-is."
...
write_tfvars() {
    ...
    # Model B: CleanArchitecture is a SEPARATE, coexisting .NET 3-tier VM set.
    # When the user selects CleanArchitecture, a fresh lab gets cleanarch-only
    # (deploy_dotnet=false avoids a duplicate-IP eShop set). For a lab that
    # already has eShop VMs in Terraform state, the state-preserve merge later
    # flips deploy_dotnet back to true so the eShop VMs are kept alongside the
    # new CleanArchitecture VMs (true coexistence). The .NET 3-tier network
    # inputs the user provided become the CleanArchitecture VM identity.
    local tf_deploy_dotnet="$DEPLOY_DOTNET"
...
deploy_dotnet = $tf_deploy_dotnet
deploy_php    = $DEPLOY_PHP
deploy_cleanarch = $tf_deploy_cleanarch
...
# --- CleanArchitecture 3-Tier IPs & Hostnames (Linux, separate set) ---
cln_fe_ip       = "${DOTNET_FE_IP:-10.1.2.40}"
cln_fe_hostname = "${DOTNET_FE_HOSTNAME:-lin-dotnet-fe-cln}"
cln_app_ip       = "${DOTNET_APP_IP:-10.1.2.41}"
cln_app_hostname = "${DOTNET_APP_HOSTNAME:-lin-dotnet-app-cln}"
cln_db_ip       = "${DOTNET_DB_IP:-10.1.2.42}"
cln_db_hostname = "${DOTNET_DB_HOSTNAME:-lin-dotnet-db-cln}"
...
# --- CleanArchitecture 3-Tier IPs & Hostnames (Windows, separate set) ---
win_cln_fe_ip       = "${WIN_DOTNET_FE_IP:-10.1.2.43}"
win_cln_fe_hostname = "${WIN_DOTNET_FE_HOSTNAME:-win-dotnet-fe-cln}"
win_cln_app_ip       = "${WIN_DOTNET_APP_IP:-10.1.2.44}"
win_cln_app_hostname = "${WIN_DOTNET_APP_HOSTNAME:-win-dotnet-app-cln}"
win_cln_db_ip       = "${WIN_DOTNET_DB_IP:-10.1.2.45}"
win_cln_db_hostname = "${WIN_DOTNET_DB_HOSTNAME:-win-dotnet-db-cln}"
...
EOF
        {
            ...
            [[ "$DEPLOY_DOTNET" == "true" && "$DEPLOY_CLEANARCH" != "true" ]] && echo "dotnet_frontend" || true
            [[ "$DEPLOY_CLEANARCH" == "true" ]] && echo "dotnet_cln_frontend" || true
            [[ "$DEPLOY_PHP" == "true" ]] && echo "php_frontend" || true
            echo ""
            echo "[appservers:children]"
            [[ "$DEPLOY_JAVA" == "true" ]] && echo "java_appserver" || true
            [[ "$DEPLOY_DOTNET" == "true" && "$DEPLOY_CLEANARCH" != "true" ]] && echo "dotnet_appserver" || true
            [[ "$DEPLOY_CLEANARCH" == "true" ]] && echo "dotnet_cln_appserver" || true
            [[ "$DEPLOY_PHP" == "true" ]] && echo "php_appserver" || true
            echo ""
            echo "[databases:children]"
            [[ "$DEPLOY_JAVA" == "true" ]] && echo "java_database" || true
            [[ "$DEPLOY_DOTNET" == "true" && "$DEPLOY_CLEANARCH" != "true" ]] && echo "dotnet_database" || true
            [[ "$DEPLOY_CLEANARCH" == "true" ]] && echo "dotnet_cln_database" || true
...
[win_dotnet_cln_frontend]
$w_dotnet_fe_ip

[win_dotnet_cln_appserver]
$w_dotnet_app_ip

[win_dotnet_cln_database]
$w_dotnet_db_ip
EOF
        [[ "$DEPLOY_PHP" == "true" ]] && cat >> "$inv_file" <<EOF || true
...
EOF
        {
            ...
            [[ "$DEPLOY_DOTNET" == "true" && "$DEPLOY_CLEANARCH" != "true" ]] && echo "win_dotnet_frontend" || true
            [[ "$DEPLOY_CLEANARCH" == "true" ]] && echo "win_dotnet_cln_frontend" || true
            [[ "$DEPLOY_PHP" == "true" ]] && echo "win_php_frontend" || true
            echo ""
            echo "[win_appservers:children]"
            [[ "$DEPLOY_JAVA" == "true" ]] && echo "win_java_appserver" || true
            [[ "$DEPLOY_DOTNET" == "true" && "$DEPLOY_CLEANARCH" != "true" ]] && echo "win_dotnet_appserver" || true
            [[ "$DEPLOY_CLEANARCH" == "true" ]] && echo "win_dotnet_cln_appserver" || true
            [[ "$DEPLOY_PHP" == "true" ]] && echo "win_php_appserver" || true
            echo ""
            echo "[win_databases:children]"
            [[ "$DEPLOY_JAVA" == "true" ]] && echo "win_java_database" || true
            [[ "$DEPLOY_DOTNET" == "true" && "$DEPLOY_CLEANARCH" != "true" ]] && echo "win_dotnet_database" || true
            [[ "$DEPLOY_CLEANARCH" == "true" ]] && echo "win_dotnet_cln_database" || true
...
run_terraform() {
    ...
    if [[ -n "$existing_resources" ]]; then
        ...
        # Model B: if CleanArchitecture VMs already exist in state, keep
        # deploy_cleanarch on so they are preserved alongside any eShop VMs.
        local tf_cleanarch="$DEPLOY_CLEANARCH"
        echo "$existing_resources" | grep -q '"cln-\|"win-cln-' && tf_cleanarch="true" || true
        if [[ "$tf_cleanarch" != "$DEPLOY_CLEANARCH" ]]; then
            step "Adjusted: deploy_cleanarch=$tf_cleanarch (existing CleanArchitecture VMs found in state)"
            sed -i "s/^deploy_cleanarch = .*/deploy_cleanarch = $tf_cleanarch/" terraform.tfvars
        fi
        ...
        # Linux 3-tier VMs
        for vm_key in java-fe java-app java-db dotnet-fe dotnet-app dotnet-db php-fe php-app php-db cln-fe cln-app cln-db; do
            echo "$existing_resources" | grep -q "vm_3tier\[\"${vm_key}\"\]" || continue
            _preserve_vm_attrs "vsphere_virtual_machine.vm_3tier" "$vm_key" ""
        done

        # Windows 3-tier VMs
        for vm_key in win-java-fe win-java-app win-java-db win-dotnet-fe win-dotnet-app win-dotnet-db win-php-fe win-php-app win-php-db win-cln-fe win-cln-app win-cln-db; do
...
EOF
    ...
    # Build extra-vars array (preserves quoting for passwords with special chars)
    local -a extra_args=(
        -e "az_migrate_user=$az_user"
        -e "az_migrate_password=$az_pass"
